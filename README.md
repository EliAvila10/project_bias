...
Motor de flujo criptográfico de alto rendimiento desarrollado en Ada y verificado formalmente con SPARK.
El motor garantiza un sesgo estadístico cero (bias-free) al mapear entropía pura en alfabetos personalizados eliminando el sesgo del módulo,
extrayendo los datos directamente de los pools del kernel del sistema operativo.

## Nivel de Verificación Formal
* **SPARK Nivel 4:** Demostración matemática completa de ausencia de errores en tiempo de ejecución (AoRTE) y cumplimiento estricto de contratos funcionales.
* **Compatibilidad de Toolchains:** Verificado de extremo a extremo tanto con GNAT/SPARK Pro 2021 como con las versiones más recientes de SPARK provistas por Alire.

## Arquitectura de Entropía Nativa
El sistema mapea las APIs de bajo nivel mediante `Interfaces.C` para suprimir capas intermedias y latencia:
* **ProcessPrng (Principal):** Consume el proveedor moderno de entropía en modo usuario (`ProcessPrng`) de Windows como fuente primaria.
* **BCryptGenRandom (Secundaria/Fallback):** Implementa una capa de respaldo robusta conectada a la API CNG tradicional (`bcrypt.dll`).

## Ecuación de truncamiento utilizada
```ada
-- Fórmula aplicada para garantizar distribución uniforme sin sesgo de módulo:
-- Formula Usada = Charset'Length * (256 / Charset'Length) - 1
-- Cualquier byte extraído > Al resultado de la formula = Rechazado
```

## Validación Empírica (Log Completo de 1 Terabyte)
Para demostrar la efectividad del algoritmo frente al sesgo,
el motor procesó y analizó un flujo masivo de **1,010,762,506,054 caracteres** (más de 1 TB de datos generados en crudo) usando un alfabeto de 69 elementos.

A continuación se adjunta el reporte bruto completo arrojado por el auditor con fecha del 2026-08-06:
...
* `src/ProcessPrng_Wrapper.ad[s|b]`: Binding nativo para el PRNG principal de Windows.
* `src/BcryptGenRandom_Wrapper.ad[s|b]`: Binding secundario de respaldo criptográfico.
* `src/Chain_Generator.ad[s|b]`: Orquestador del flujo y lógica de descarte.
* `src/Math_Functions.ad[s|b]`: Definición de contratos matemáticos bajo SPARK.
